...
> **分位基准（v3 · 2026-09 回测定稿）**：默认采用**纯全池横比**（`industry_blend=0.0`）。原因——对 2021-09~2026-09 做了点位回溯回测（等权 Top20、季度再平衡），纯全池年化 **10.8% / 夏普 0.71** 全面优于纯行业内（7.4%/0.42）与混合 0.7（5.9%/0.39）。该区间恰逢高股息/中特估行情，银行整组天然跑赢，"银行霸榜"不是缺陷而是有效的便宜信号。同时纯全池下"小行业组当第一拿满分"的问题**天然消失**（不再于 2 只组内排第一）。代码保留 `within_industry_pct`/`blend_pct` 与 `industry_blend` 开关，可随时切回行业内或混合。
...
### 5. 半年报体检（财报解读层，独立不污染打分）

半年报陆续披露后新增的一层**独立解读**，对每只上榜蓝筹取最新一期财报（优先 2026 半年报，未披露回退一季报 / 年报），
给出「利好 / 利空 / 中性 / 未出」结论。**四维总分保持纯净（不被财报判断污染）**，但结论会进入**排序分**：判定「利空」的标的在有效排名中扣 10 分自动沉底，便于一眼避坑；四维权重本身不被改动，决策仍由人做。排序分即页面默认排序依据。

- **数据源**：东方财富业绩报表（`ak.stock_yjbb_em`），一次返回全市场，按代码匹配。
- **主表新列「财报」**：利好（绿）/ 利空（红）/ 中性（灰）/ 未出（浅灰），可点过滤栏「财报利好 / 财报利空」一键筛。
- **解读面板**：深研展开顶部、以及主表行展开详情里，均给出营收同比 / 净利同比 / ROE / 毛利率 / EPS + 利好点 / 风险点。
- **判定（可核验、不接 LLM）**：
  - 净利同比 ≥20% 加分、＜-10% 扣分；营收正而净利负 → 判利空并提示「增收不增利」。
  - 净利增速快于营收 +5pct 加分；ROE ≥15% 加分、＜0 扣分。
  - 毛利率 ≥40% 加分、＜15% 扣分（**金融业豁免**）；每股经营现金流为负扣分（**金融业豁免**）。
  - 综合分 ≥1 → 利好；≤-1 → 利空；其余 → 中性。
- **金融业豁免**：银行 / 保险 / 证券的高杠杆、毛利率、现金流口径不可比，相关指标自动跳过，只看净利与 ROE。
- **优雅降级**：取数失败 → 全部回退为「未出」，页面照常工作，绝不编造数值。
