...
CONFIG = {
    ...
    # 0.0 = 纯全池（银行整组天然便宜 → 霸榜，但回测(2021-09~2026-09)证明收益/夏普最优）
    # 0.7 = 混合：掐小池塘泡沫 + 保留绝对便宜信号，但回测显示收益/夏普均低于纯全池
    # 结论（2026-09 回测）：纯全池年化10.8%/夏普0.71 碾压各档；
    #       且在纯全池下"小行业满分"问题天然消失（不再于2只组内排第一拿100分），故采用 0.0
    "industry_blend": 0.0,
